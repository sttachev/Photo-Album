...
export default function Album3(props) {
    const [photos, setPhotos] = useState([]);
    const filteredPhotos = photos.filter(photo => photo.albumId === 2);
    ...
    return (<>
        <div>
            <h1>Album 3 - Photos {filteredPhotos.length}</h1>
            <div className="container">
                <div className="row">
                    <div className="col-12 row">
                        {favorites.filter(photo => photo.albumId === 3).map(item => {
                            return (
                                <Photo
                                    key={item.id}
                                    item={item}
